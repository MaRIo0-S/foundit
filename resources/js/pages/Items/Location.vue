...
function applyFilters(page = 1) {
    ...
    router.get(route("locations.show", { id: props.location.id }), params, {
        preserveScroll: true,
        preserveState: true,
    });
}
...
const currentPage = computed(() => props.location.items?.current_page ?? 1);
const lastPage = computed(() => props.location.items?.last_page ?? 1);
...
function isOwnItem(item) {
    return user.value && item.user_id === user.value.id;
}
...
function claimButtonLabel(item) {
    ...
    if (isOwnItem(item)) return "Votre déclaration";
    return "Réclamer";
}
</script>
...
<template>
    <div class="location-page">
        <div class="location-page__inner container">
            <div class="location-page__topbar">
                <button
                    class="back-btn"
                    type="button"
            ...
            <div class="location-page__header">
                <div class="location-page__header-left">
                    <span class="location-page__icon">
                    ...
                    <div>
                        ...
                        <p class="location-page__subtitle">
                            {{ location.items?.total ?? 0 }} objet{{
                                (location.items?.total ?? 0) !== 1 ? "s" : ""
                            }}
                            trouvé{{
                                (location.items?.total ?? 0) !== 1 ? "s" : ""
                            }}
            ...
            <div v-if="location.items?.data?.length > 0" class="items-grid">
                <div
                    class="item-card"
                    v-for="item in location.items.data"
                    :key="item.id"
                >
            ...
            <div v-else class="empty-state">
                ...
                <p class="empty-state__text">
                    Aucun objet ne correspond à votre recherche pour ce lieu.
                </p>
            </div>
                    ...
                    <div class="form-group">
                        ...
                        <p class="form-notice">
                            ...
                            <span
                                ...
                                marque, gravures, circonstances — sous peine de
                                rejet.</span
                            >
                        </p>
                        <textarea
                    ...
                    <div class="form-group">
                        ...
                        <p class="form-notice">
                            ...
                            <span
                                ...
                                au déclarant uniquement après approbation.</span
                            >
                        </p>
                        <p
...
/* --- Added Filter & Pagination CSS rules shared from Dashboard --- */
.filters {
    display: flex;
    flex-direction: column;
    gap: 0.75rem;
